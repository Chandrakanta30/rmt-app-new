                        ...
                        <?php if ($canModify): ?>
                            <td style="padding: 1rem;">
                                <?php if (has_permission('update_asrno')): ?>
                                    <button type="button" class="btn btn-ghost" onclick="openAsrEditModal(<?= (int) $asr['id'] ?>, '<?= esc($asr['asr_no'], 'js') ?>', <?= (int) $asr['form_id'] ?>)">Edit</button>
                                <?php endif; ?>
...
<?php if (has_permission('update_asrno')): ?>
<div id="asrEditModal" style="display: none; position: fixed; inset: 0; background: rgba(15,23,42,0.5); align-items: center; justify-content: center; z-index: 1100;">
    <div class="protocol-card" style="width: 100%; max-width: 480px; margin: 0 1rem;">
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom: 1.25rem;">
            <h3 style="margin:0;">Edit ASR No.</h3>
            <button type="button" onclick="closeAsrEditModal()" style="border:none; background:none; font-size:1.25rem; cursor:pointer; color:#7f8c8d;">&times;</button>
        </div>
        ...
        <form id="asrEditForm" method="POST">
            ...
            <div class="form-group">
                ...
                <textarea id="update_remark" name="update_remark" rows="3" required placeholder="Why is this ASR No. being updated?"></textarea>
            </div>

            <div style="display: flex; gap: 12px; margin-top: 1.5rem;">
                <button type="submit" class="btn btn-success" style="flex: 1;">Save Changes</button>
                <button type="button" class="btn btn-ghost" style="flex: 1;" onclick="closeAsrEditModal()">Cancel</button>
            </div>
        </form>
    </div>
</div>
...
<div id="asrActionSuccessModal" style="display: flex; position: fixed; inset: 0; background: rgba(15,23,42,0.5); align-items: center; justify-content: center; z-index: 1100;">
    <div class="protocol-card" style="width: 100%; max-width: 380px; margin: 0 1rem; text-align: center; padding: 2rem;">
        ...
        <button type="button" class="btn btn-success" style="width: 100%;" onclick="document.getElementById('asrActionSuccessModal').style.display='none'">OK</button>
    </div>
</div>
<?php endif; ?>
