...
<?= $this->section('title') ?>User Audit Log<?= $this->endSection() ?>
...
<div class="audit-page">
    <div class="audit-header">
        <div>
            <h2>Audit Log &mdash; <?= esc($user['name']) ?></h2>
            <div class="subtitle">User ID: #<?= esc($user['id']) ?> &bull; Email: <?= esc($user['email']) ?></div>
        </div>
        <a class="btn btn-ghost" href="<?= base_url('users') ?>">&larr; Back to Users</a>
    </div>
                    ...
                    <?php else: ?>
                        <?php foreach ($changes as $change): ?>
                            <tr>
                                ...
                                <td class="audit-user"><?= esc($change['field'] ?? '-') ?></td>
            ...
            <div class="audit-pagination">
                ...
                <div style="display: flex; gap: 8px;">
                    <?php if ($pagination['page'] > 1): ?>
                        <a class="btn btn-ghost" href="<?= base_url('users/audit-log/' . $user['id']) ?>?page=<?= $pagination['page'] - 1 ?>">&larr; Previous</a>
                    <?php else: ?>
                        <span class="btn btn-ghost disabled">&larr; Previous</span>
                    <?php endif; ?>

                    <?php if ($pagination['page'] < $pagination['totalPages']): ?>
                        <a class="btn btn-ghost" href="<?= base_url('users/audit-log/' . $user['id']) ?>?page=<?= $pagination['page'] + 1 ?>">Next &rarr;</a>
                    <?php else: ?>
